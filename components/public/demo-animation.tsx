    ...
    <div className="relative">
      ...
      <div className="max-w-6xl w-full mx-auto">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex items-center gap-2 px-3 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-widest bg-primary/10 text-primary mb-2">
            <Zap size={10} /> Synchronisation Intelligente
          </div>
          <h2 className="text-xl md:text-2xl font-extrabold text-foreground mb-2 text-balance">
            L&apos;automatisation qui ne se trompe pas.
          </h2>
          <p className="text-muted-foreground text-sm max-w-lg mx-auto leading-relaxed">
            Formly valide la cohérence de chaque champ pour garantir la qualité de votre saisie.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
          ...
          {/* SOURCE */}
          <div className="bg-card rounded-xl shadow-lg p-4 border border-border flex flex-col">
            <div className="flex items-center gap-2 mb-4 pb-2 border-b border-border">
              <div className="p-2 rounded-lg bg-primary text-primary-foreground shadow-md shadow-primary/25">
                <Shield size={18} />
              </div>
              <h3 className="font-bold text-base text-foreground">Source Formly</h3>
            </div>
              ...
              {stepsData.map((step, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`p-2.5 rounded-xl border transition-all duration-300 ${isActive
                  ...
                  >
                    <div className="text-[9px] font-bold text-muted-foreground uppercase mb-0.5">{step.label}</div>
                    <div className="font-bold text-base text-foreground">{step.value}</div>
                  </div>
                )
              })}
            </div>
            ...
            <Button
              ...
              className={`w-full h-10 rounded-lg font-bold text-xs ${demoState === 'running'
          ...
          {/* PORTAL A */}
          <div className="bg-card rounded-xl shadow-lg border-t-2 border-blue-600 flex flex-col border border-blue-600/20 overflow-hidden">
            <div className="px-4 py-2 border-b border-blue-600/20 bg-blue-600/15 flex items-center justify-between">
              <div className="flex items-center gap-2 text-blue-600">
                <Globe size={12} />
                <span className="font-bold text-[9px] uppercase tracking-wider">Portail Assureur A</span>
              </div>
            </div>
              ...
              {stepsData.map((step, idx) => {
                ...
                return (
                  <div key={idx} ref={el => { targetARefs.current[idx] = el }}>
                    <label className="block text-[12px] font-bold text-muted-foreground uppercase mb-1">{step.targetA}</label>
                    <div className={`h-8 rounded-lg border flex items-center px-3 transition-all duration-300 ${isError
                        ? 'bg-destructive/5 border-destructive animate-shake'
                        : isFilled
                          ? 'bg-transparent border-blue-600/50'
          ...
          {/* PORTAL B */}
          <div className="bg-card rounded-xl shadow-lg border-t-2 border-red-500 flex flex-col border border-red-500/20 overflow-hidden">
            <div className="px-4 py-2 border-b border-red-500/20 bg-red-500/15 flex items-center gap-2">
              <Globe size={12} className="text-red-500" />
              <span className="font-bold text-[9px] uppercase tracking-wider text-red-600">Portail Assureur B</span>
            </div>
            <div className="p-4 space-y-2 flex-1">
              {stepsData.map((step, idx) => {
                ...
                return (
                  <div key={idx} ref={el => { targetBRefs.current[idx] = el }}>
                    <label className="block text-[12px] font-bold text-muted-foreground uppercase mb-1">{step.targetB}</label>
                    <div className={`h-8 rounded-lg border flex items-center px-3 transition-all duration-500 ${isFilled
                        ? 'bg-transparent border-blue-600/50'
                        : 'bg-transparent border-border/40'
                      }`}>
                      {isFilled ? (
        ...
        {/* Footer badges */}
        <div className="mt-8 text-center text-muted-foreground text-[10px] font-bold uppercase tracking-widest flex items-center justify-center gap-3 flex-wrap">
